(function () {
    ...
    _.forEach(data, function(el) {
            _.forEach(el.skills, function(skill) {
                if(!skills[skill]) {
                    skills[skill] = skill;
                }
            });
    ...
    for(var s in skills) {
        skillsArr.push(s.toLowerCase());
    }

    var skillsArrSorted = _.sortBy(skillsArr);


    // arr of names in asc order
    var personsSorted = _.chain(data)
        .map(function(el) {
            return {
                friends: el.friends.length,
                name: el.name}
        })
        .sortBy('friends')
        .map(function(el) {
    ...
    _.forEach(data, function(el) {
        return _.forEach(el.friends, function(friend) {
            if(!names[friend.name]) {
                names[friend.name] = friend.name;
            }
        });
    });
    ...
    for(n in names) {
        uniqueFriends.push(n);
    }

    // results
    console.log('task 1: ', skillsArrSorted);
    console.log('task 2: ',personsSorted);
    console.log('task 3: ',uniqueFriends);
}());